...
class InsuranceTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let customer_id = insurance.customer_id
        let customer = mainVC?.customer_table.first(where: { $0.id == customer_id })
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete,let mainVC = mainVC  {
            ...
            let claims = mainVC.claim_table.filter { $0.policy_id == insurance.id }
            for claim in claims {
                mainVC.claim_table.remove(at: mainVC.claim_table.firstIndex(of: claim)!)
                mainVC.managedContext.delete(claim)
            }

            let payments = mainVC.payment_table.filter { $0.policy_id == insurance.id }
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        if searchText.isEmpty {
            ...
            filteredResults = mainVC?.insurance_table.filter { insurance in
                let customer_id = insurance.customer_id
                let customer = mainVC?.customer_table.first(where: { $0.id == customer_id })
                return  String(insurance.id).lowercased().contains(searchText.lowercased()) ||
                customer?.name?.lowercased().contains(searchText.lowercased()) ?? false
            } ?? []
        }
        if(filteredResults.isEmpty) {
            let alert = UIAlertController(title: "No Results", message: "No results found for the search", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
            present(alert, animated: true, completion: nil)
        }
        tableView.reloadData()
