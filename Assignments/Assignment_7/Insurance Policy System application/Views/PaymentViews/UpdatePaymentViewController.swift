...
class UpdatePaymentViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let policyID = mainVC?.payments[(indexPath?.row)!].policy_id
...
//        status can only be Processing or Processed
        ...
        // Update payment
        mainVC.payments[indexPath.row].payment_amount = paymentAmountDouble
        mainVC.payments[indexPath.row].status = paymentStatus ?? "Processing"
        mainVC.payments[indexPath.row].payment_method = paymentMethod ?? "Debit Card"
